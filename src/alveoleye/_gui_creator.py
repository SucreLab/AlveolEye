...
@typechecked
def create_sub_layout(layout: QLayout, elements: Sequence[QWidget | QLayout]) -> QLayout:
...
def create_label_and_spin_box_layout(
        label_text: str,
        tooltip_text: str,
        spin_box_min: int | float,
        spin_box_max: int | float,
        spin_box_default: int | float,
        spin_box_step: int | float,
        spin_box_suffix: str,
        value_type: str = "single",
        decimals: int = 5
) -> tuple[QHBoxLayout, QLineEdit, QDoubleSpinBox | QSpinBox]:
    label = QLineEdit(label_text)
    label.setReadOnly(True)
    label.setObjectName("labelLineEdit")
    spin_box: QDoubleSpinBox | QSpinBox
...
def create_check_box_and_spin_box_layout(
        ...
        value_type: str = "single",
        decimals: int = 5
) -> tuple[QHBoxLayout, QCheckBox, QDoubleSpinBox | QSpinBox]:
...
@typechecked
def create_confirmation_message_box(parent: QWidget | None, message: str) -> bool:
...
@typechecked
def toggle(state: bool, elements: QWidget | QLayout | Sequence[QWidget | QLayout]) -> None:
    if not isinstance(elements, (list, tuple)):
        elements = [elements]
    for item in elements:
        stack: list[QWidget | QLayout] = [item]
        while stack:
            current = stack.pop()
